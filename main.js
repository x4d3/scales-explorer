...
const SCALES = {
    "Major": {
        intervals: MAJOR_INTERVALS, startNote: "C", startKey: "C"
    }, "Minor": {
        intervals: MINOR_INTERVALS, startNote: "C", startKey: "Eb"
    }, "Dorian": {
        intervals: MAJOR_INTERVALS, startNote: "G", startKey: "F"
    }
};
...
const generatesScale = (note, intervals, accidentals) => {
    ...
    const notes = [];
    for (let i = 0; i < 15; i++) {
        const staveNote = new Vex.Flow.StaveNote({keys: [`${note}/${scale}`], duration: "4d"});
        ...
        if ((note.slice(0, 1) == "B" || note.slice(0, 1) == "A") && (nextNote.slice(0, 1) == "C" || nextNote.slice(0, 1) == "D")) {
            scale++;
        }
        note = nextNote;
    }
...
const parseIntOrDefault = (param, defaultValue) => {
    if (param) {
